...
import { TbHome, TbUserCircle, TbSettings, TbInfoCircle, TbChevronLeft, TbChevronRight, TbChevronsLeft, TbChevronsRight } from 'react-icons/tb';
...
const SubCategoryPage = () => {
  ...
  const handleSubSubmit = (e) => {
    ...
    const nextLetter = String.fromCharCode(97 + existingSubsForParent.length);
  ...
  const getSubTableData = () => {
    return subCategories.map(sub => {
      ...
      return [
        ...
        isEditing ?
          `<button class="btn btn-sm btn-success me-1" onclick="saveSubCategory('${sub.id}')">✓</button><button  class="btn btn-sm btn-secondary" onclick="cancelSubEdit()">✕</button>` :
          `<button style="padding: 0.2rem 0.2rem;" class="btn btn-sm btn-primary me-1" onclick="editSubCategory('${sub.id}', '${sub.name}')">${ReactDOMServer.renderToStaticMarkup(<FiEdit />)}</button><button  style="padding: 0.2rem 0.2rem;" class="btn btn-sm btn-danger" onclick="deleteSubCategory('${sub.id}')">${ReactDOMServer.renderToStaticMarkup(<MdDeleteOutline />)}</button>`
      ];
    });
  };
  ...
  return (
    <div className='mt-4'>
      <ComponentCard title="Sub Category List" isCollapsible>
